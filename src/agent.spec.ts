...
describe("MakerDAO’s Bridge Invariant checks", () => {
  ...
  it("returns empty findings if the optimism layer 2 dai supply is equal to the layer 1 escrow dai balance", async () => {
    mockProvider.setNetwork(networkOChainID);
    await initialize();
    handleBlock = provideHandleBlock(provider, mockGetAlerts);
    const blockEvent = createBlockEvent({
    ...
    mockProvider.addCallTo(
      ...
      {
        inputs: [],
        outputs: [MOCK_VALUES_1.OPTIMISM_L1_ESCROW_BAL], // Equal to L1 balance
      },
    );
  ...
  it("returns empty findings if there are no relevant alerts", async () => {
  ...
  it("returns empty findings if not on the Ethereum mainnet", async () => {
    mockProvider.setNetwork(networkOChainID);
    await initialize();
    handleBlock = provideHandleBlock(provider, mockGetAlerts);
    const blockEvent = createBlockEvent({
  ...
  it("returns findings based on parsed alerts", async () => {
    mockProvider.setNetwork(1);
    await initialize();
    handleBlock = provideHandleBlock(provider, mockGetAlerts);
    const blockEvent = createBlockEvent({
    ...
    mockProvider
      .addCallTo(DAI_ADDRESS, blockNumber, L1_PROXY, "balanceOf", {
        inputs: [L1_ESCROW_OPTIMISM],
        outputs: [MOCK_VALUES_2.OPTIMISM_L1_ESCROW_BAL],
      })
      .addCallTo(DAI_ADDRESS, blockNumber, L1_PROXY, "balanceOf", {
        inputs: [L1_ESCROW_ARBITRUM],
        outputs: [MOCK_VALUES_2.ARBITRUM_L1_ESCROW_BAL],
      });
    ...
    const expectedFindings = Finding.fromObject({
      name: `Combined supply of optimism and Arbitrum MakerDao escrows on layer 1`,
      description: `Escrow balances: Arbitrum: ${MOCK_VALUES_2.ARBITRUM_L1_ESCROW_BAL} Optimism: ${MOCK_VALUES_2.OPTIMISM_L1_ESCROW_BAL}`,
      alertId: "new block check escrows on layer 1",
      severity: FindingSeverity.Info,
      type: FindingType.Info,
      protocol: "Ethereum",
      metadata: {
        escrowBalanceOptimism: `${MOCK_VALUES_2.OPTIMISM_L1_ESCROW_BAL}`,
        escrowBalanceArbitrum: `${MOCK_VALUES_2.ARBITRUM_L1_ESCROW_BAL}`,
      },
    });
